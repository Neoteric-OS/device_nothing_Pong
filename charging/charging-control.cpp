...
string ChargeStatusListener::read_line(const char *file) {
    ...
    if (!input_file.is_open())
        ALOGE("open %s failed", file, errno);
...
void ChargeStatusListener::limitCharge() {
    while (enableChgLimit) {
        ...
        int i, temp_threshold, current_volt_limit;
        ...
        if (usbChgActive) {
            ...
            if (usingPps) {
                ...
                temp_threshold = ppsTempThresholds[i];
            } else {
                ...
                temp_threshold = qcTempThresholds[i];
            }
            ...
            temp_threshold = wlsTempThresholds[i];
        }

        current_volt_limit = CHG_VOLT_TBL[volt_limit_index];
        if (current_volt_limit != last_volt_limit) {
            ALOGD("Temp threshold %d , limit charging to %d mA, last charging limit %d mA", 
                temp_threshold, current_volt_limit, last_volt_limit);
...
ndk::ScopedAStatus ChargeStatusListener::healthInfoChanged(const HealthInfo& info) {
    ALOGI("healthInfoChanged: %d", info.batteryStatus);
